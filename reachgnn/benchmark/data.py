...
class Radical_TSDG_Reaction_HeteroGraph_DataSet(Base_Reaction_HeteroGraph_DataSet):
    _urls = "XXXXXX"  # 'https://ndownloader.figshare.com/files/3195404'
    ...
    def __init__(self, root: str, transform: Optional[Callable] = None,
        ...
        if True:
            ...
            self.bond_types = []
        if True:
            self.atom_types =  ['H', 'C', 'N', 'O', 'F', 'S']
            self.charge_types =  [0, 1]
            self.degree_types =  [1, 2, 3, 4]
            self.hybridization_types =  ['S', 'SP', 'SP2', 'SP3']
            self.hydrogen_types =  [0, 1, 2, 3]
            self.valence_types =  [1, 2, 3, 4]
    ...
    @property
    def raw_file_names(self) -> List[str]:
        return ['Radical_TSDG/Canonicalized_Radical_TSDG_input_data.csv', 'Radical_TSDG/sdf/']

    @property
    def processed_file_names(self) -> str:
        return 'Radical_TSDG_Reaction_HeteroGraph_DataSet.pt'

    def download(self):
        pass
        # raise NotImplementedError('please download and unzip dataset from %s, and put it at %s' % (self._urls, self.raw_dir))

class Buchwald_Hartwig_Reaction_HeteroGraph_DataSet(Base_Reaction_HeteroGraph_DataSet):
    _urls = "XXXXXX"  # 'https://ndownloader.figshare.com/files/3195404'
    ...
    def __init__(self, root: str, transform: Optional[Callable] = None,
        ...
        if True:
            ...
            self.bond_types = []
        if True:
            self.atom_types =  ['H', 'C', 'N', 'O', 'F', 'S', 'P', 'Cl', 'Br', 'I', 'Pd']
            self.charge_types =  [0]
            self.degree_types =  [1, 2, 3, 4]
            self.hybridization_types =  ['S', 'SP2', 'SP3', 'SP3D', 'UNSPECIFIED']
            self.hydrogen_types =  [0, 1, 2, 3]
    ...
    @property
    def raw_file_names(self) -> List[str]:
        return ['Buchwald_Hartwig/Canonicalized_Buchwald_Hartwig_input_data.csv', 'Buchwald_Hartwig/sdf/']

    @property
    def processed_file_names(self) -> str:
        return 'Buchwald_Hartwig_Reaction_HeteroGraph_DataSet.pt'

    def download(self):
        pass
        # raise NotImplementedError('please download and unzip dataset from %s, and put it at %s' % (self._urls, self.raw_dir))

class Imine_Thiol_CPA_EnantioSel_Reaction_HeteroGraph_DataSet(Base_Reaction_HeteroGraph_DataSet):
    _urls = "XXXXXX"  # 'https://ndownloader.figshare.com/files/3195404'
    ...
    def __init__(self, root: str, transform: Optional[Callable] = None,
        ...
        if True:
            ...
            self.bond_types = []
        if True:
            self.atom_types =  ['H', 'C', 'N', 'O', 'F', 'S', 'Cl', 'P', 'Br', 'Si']
            self.charge_types =  [0]
            self.degree_types =  [1, 2, 3, 4, 6]
            self.hybridization_types =  ['S', 'SP2', 'SP3', 'SP3D2']
            self.hydrogen_types =  [0, 1, 2, 3]
            self.valence_types =  [1, 2, 3, 4, 5, 6]
            self.bond_types =  ['SINGLE', 'DOUBLE', 'TRIPLE', 'AROMATIC']             
        super().__init__(root, transform, pre_transform, pre_filter)
    ...
    @property
    def raw_file_names(self) -> List[str]:
        return ['Imine_Thiol_CPA_EnantioSel/Canonicalized_Imine_Thiol_CPA_EnantioSel_input_data.csv', 'Imine_Thiol_CPA_EnantioSel/sdf/']

    @property
    def processed_file_names(self) -> str:
        return 'Imine_Thiol_CPA_EnantioSel_Reaction_HeteroGraph_DataSet.pt'

    def download(self):
        pass
        # raise NotImplementedError('please download and unzip dataset from %s, and put it at %s' % (self._urls, self.raw_dir))

class Suzuki_Miyaura_Reaction_HeteroGraph_DataSet(Base_Reaction_HeteroGraph_DataSet):
    _urls = "XXXXXX"  # 'https://ndownloader.figshare.com/files/3195404'
    ...
    def __init__(self, root: str, transform: Optional[Callable] = None,
        ...
        if True:
            ...
            self.bond_types = []
        if True:
            self.atom_types =  ['H', 'C', 'N', 'O', 'F', 'S', 'Cl', 'Br', 'I', 'B', 'K', 'Na', 'Cs', 'P', 'Li', 'Pd', 'Fe']
            self.charge_types =  [-1, 0, 1]
            self.degree_types =  [0, 1, 2, 3, 4, 10]
            self.hybridization_types =  ['S', 'SP', 'SP2', 'SP3', 'SP3D', 'SP3D2', 'UNSPECIFIED']
            self.hydrogen_types =  [0, 1, 2, 3]
            self.valence_types =  [0, 1, 2, 3, 4, 5, 6, 10]
            self.bond_types =  ['SINGLE', 'DOUBLE', 'TRIPLE', 'AROMATIC', 'DATIVE']
        super().__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def raw_file_names(self) -> List[str]:
        return ['Suzuki_Miyaura/Canonicalized_Suzuki_and_Miyaura_input_data.csv', 'Suzuki_Miyaura/sdf/']

    @property
    def processed_file_names(self) -> str:
        return 'Suzuki_Miyaura_Reaction_HeteroGraph_DataSet.pt'
...
def DataSplit(dataset, batch_size, test_train_split=0.60, val_train_split=0.10, 
              run_shuffle=False, random_seed=42, with_val=False):
    dataset_size = len(dataset)
    indices = np.arange(dataset_size)

    test_split = int(
        np.floor((test_train_split+val_train_split) * dataset_size))
    val_split = int(np.floor(test_train_split * dataset_size))
    ...
    if with_val:
        ...
        print("test_split:", test_split, "val_split:",val_split)
    else:
...
def DataSplit_test_BH(dataset, test_idx, batch_size, file_pth='../datasets/raw/Buchwald_Hartwig/Test_4_split.csv'):
    df = pd.read_csv(file_pth, index_col=0)
    assert len(dataset)==len(df)

    train_indices = (df['Test%d_flag'%test_idx]=='train').values.nonzero()[0]
    test_indices = (df['Test%d_flag'%test_idx]=='test').values.nonzero()[0]

    print("n_train", len(train_indices), "n_test", len(test_indices))
    train_loader = DataLoader(dataset[train_indices], batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(dataset[test_indices], batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(dataset[test_indices], batch_size=batch_size, shuffle=False)
    return train_loader, val_loader, test_loader

def DataSplit_test_DE(dataset, batch_size, file_pth='../datasets/raw/Imine_Thiol_CPA_EnantioSel/TrainTest_384_216_304_171_split.csv'):
    df = pd.read_csv(file_pth, index_col=0)
    assert len(dataset)==len(df)

    train_indices = (df['flag']=='train').values.nonzero()[0]
    test_sub_indices = (df['flag']=='test_sub').values.nonzero()[0]
    test_cat_indices = (df['flag']=='test_cat').values.nonzero()[0]
    test_subcat_indices = (df['flag']=='test_sub-cat').values.nonzero()[0]
    test_indices = np.concatenate((test_sub_indices, test_cat_indices, test_subcat_indices))

    print("n_train",len(train_indices), "n_test_sub", len(test_sub_indices), "n_test_cat", len(test_cat_indices), "n_test_sub_cat", len(test_subcat_indices))
    train_loader = DataLoader(dataset[train_indices], batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(dataset[test_indices], batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(dataset[test_indices], batch_size=batch_size, shuffle=False)
    return train_loader, val_loader, test_loader
