...
class Trainer(object):
    def __init__(self, PATH=None, model=None, train_loader=None, val_loader=None,
                 test_loader=None, criterion=None, optimizer=None, device=None, lr_scheduler=None):
        if device:
            model.to(device)
    ...
    def run(self, epochs=1, saved_step=200, log_d_step=50, print_step=5, print_train_log=False):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.epoch_ % print_step == 0:
                ...
                print("Eopch: %4d  Loss: %.6f, Val_Loss: %.6f, train_time: %.4fs, val_time: %.4fs," %
                      (self.epoch_, self.train_loss, self.val_loss, train_time, val_time))
...
def LearningCurve(epoch, train_loss, val_loss, title='Learning Curve', figure_file=None):
    ...
    # plt.subplots_adjust(wspace =0.3, hspace =0.3) #Adjust subplot spacing
